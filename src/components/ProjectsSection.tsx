...
export function ProjectsSection() {
  const featuredByCategory = projectCategories.flatMap((category) =>
    projects.filter((project) => project.category === category && project.featured).slice(0, 1),
  );
  ...
  return (
    <section id="projects" className="space-y-6">
      ...
      <div className="space-y-5">
        {featuredByCategory.map((project) => (
          <article
            key={project.title}
            className="overflow-hidden rounded-2xl border border-white/60 bg-white/60 transition hover:-translate-y-1 hover:bg-white"
          >
            <div className={`h-2 w-full bg-gradient-to-r ${project.accent}`} />
            <div className="relative w-full" style={{ aspectRatio: "16 / 9" }}>
              <Image src={project.image ?? "/project-placeholder.svg"} alt={project.title} fill className="object-cover" />
            </div>
            <div className="space-y-4 p-6">
              ...
              <div className="group/summary">
                <p className="text-slate-600">{truncateSummary(project.summary, 50)}</p>
                <div className="mt-2 max-h-0 overflow-hidden rounded-xl border border-transparent bg-white/90 px-3 text-sm leading-relaxed text-slate-700 opacity-0 transition-all duration-200 group-hover/summary:max-h-40 group-hover/summary:border-sky-200 group-hover/summary:py-3 group-hover/summary:opacity-100">
                  {project.summary}
                </div>
              </div>
